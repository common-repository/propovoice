...
class Webhook {
    ...
    public function routes() {
        ...
        register_rest_route(
            'ndpv/v1', '/webhooks' . ndpv()->plain_route(), [
				'methods' => 'GET',
				'callback' => [ $this, 'get' ],
				'permission_callback' => [ $this, 'get_per' ],
			]
    ...
    public function get( $req ) {
        ...
        while ( $query->have_posts() ) {
            ...
            if ( isset( $query_meta['active'] ) && $query_meta['active'][0] ) {
            ...
            $query_data['url'] = isset( $query_meta['url'] )
                ? $query_meta['url'][0]
                : '';
            $query_data['method'] = isset( $query_meta['method'] )
                ? $query_meta['method'][0]
                : '';
            $query_data['actions'] = isset( $query_meta['actions'] )
                ? maybe_unserialize( $query_meta['actions'][0] )
                : [];
