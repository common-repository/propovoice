...
class Team {
    ...
    public function routes() {
        ...
        register_rest_route(
            'ndpv/v1', '/teams' . ndpv()->plain_route(), [
				'methods' => 'GET',
				'callback' => [ $this, 'get' ],
				'permission_callback' => [ $this, 'get_per' ],
			]
    ...
    public function create( $req ) {
        ...
        if ( $reg_errors->get_error_messages() ) {
            ...
            if ( ! $user_id ) {
                $password = wp_generate_password( $length = 12, $is_standard_special_chars = false );
                $user_args = [
                    'user_login'     => $email,
                    'user_pass'      => $password,
                    'user_email'     => $email,
                ...
                $check_roles = [ 'ndpv_admin', 'ndpv_manager', 'ndpv_staff' ];
                $role_exist = false;
            ...
            do_action( 'ndpvp_webhook', 'staff_add', $param );
    ...
    public function get_per() {
        return current_user_can( 'manage_options' ) || current_user_can( 'ndpv_admin' ) || current_user_can( 'ndpv_manager' ) || current_user_can( 'ndpv_staff' );
    }

    public function create_per() {
        return current_user_can( 'manage_options' ) || current_user_can( 'ndpv_admin' ) || current_user_can( 'ndpv_manager' );
    }
