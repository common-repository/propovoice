...
class File {
    ...
    public function routes() {
        ...
        register_rest_route(
            'ndpv/v1', '/files' . ndpv()->plain_route(), [
				'methods' => 'GET',
				'callback' => [ $this, 'get' ],
				'permission_callback' => [ $this, 'get_per' ],
			]
    ...
    public function get( $req ) {
        ...
        while ( $query->have_posts() ) {
            ...
            $query_data['type'] = isset( $query_meta['type'] )
                ? sanitize_text_field( $query_meta['type'][0] )
                : '';

            $query_data['url'] = isset( $query_meta['url'] )
                ? sanitize_text_field( $query_meta['url'][0] )
                : '';

            $file_id = isset( $query_meta['file'] )
                ? sanitize_text_field( $query_meta['file'][0] )
                : '';

            $query_data['title'] = isset( $query_meta['title'] )
                ? sanitize_text_field( $query_meta['title'][0] )
            ...
            if ( $file_id ) {
                ...
                if ( $attach_type == 'application/pdf' ) {
                    ...
                    if ( $file_src ) {
                        ...
                        $file_data['src'] = $file_src[0];
                        $file_data['src_small'] = $file_src;
                    }
                }
            }
            $query_data['file'] = $file_data;

            $author_id = get_post_field( 'post_author', $id );
            $query_data['upload_by'] = get_avatar_url(
                $author_id, [
					'size' => '36',
				]
            );
    ...
    public function create( $req ) {
        ...
        if ( $reg_errors->get_error_messages() ) {
            ...
            if ( ! is_wp_error( $post_id ) ) {
                ...
                $param['id'] = $post_id;
                do_action( 'ndpvp_webhook', 'file_add', $param );
