...
class Style {
    ...
    public function get_plugin_screen() {
        ...
        if ( $current_screen->id === 'toplevel_page_ndpv' && isset( $_GET['page'] ) && sanitize_text_field( wp_unslash( $_GET['page'] ) ) === 'ndpv' ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
    ...
    public function clear_styles_and_scripts() {
        global $wp_styles;
        $styles_to_keep = [ 'ndpv-admin', 'ndpv-style', 'ndpv-est-inv', 'ndpv-google-font', 'admin-bar', 'wp-auth-check', 'plugin-installer-style', 'colors' ];
